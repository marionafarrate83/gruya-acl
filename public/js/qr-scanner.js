class QRScanner {
    ...
    checkScannerAvailability() {
        if (!this.isScannerAvailable) {
            ...
            document.getElementById('start-scan').innerHTML = '<i class="fas fa-ban"></i> Escáner No Disponible';
        }
    }
    ...
    async handleScanResult(result) {
        const visitId = result ? result.trim() : '';
        if (!visitId) return;
    ...
    showValidResult(visitor) {
        ...
        detailsDiv.innerHTML = `
            ...
            <p><strong>Residencia:</strong> ${visitor.residentId.residenceNumber}</p>
            <p><strong>Hora:</strong> ${visitor.visitTime}</p>
            <p><strong>Motivo:</strong> ${visitor.visitReason}</p>
            <button class="btn btn-info btn-sm mt-2" onclick="window.qrScanner.showVisitorDetails('${visitor._id}')">
    ...
    async showVisitorDetails(visitorId) {
        try {
            ...
            modalDetails.innerHTML = `
                <div class="row">
                    <div class="col-6">
                        <p><strong>ID Visita:</strong><br><code>${visitor.visitId}</code></p>
                        <p><strong>Nombre:</strong><br>${visitor.visitorName}</p>
                        <p><strong>Teléfono:</strong><br>${visitor.visitorPhone}</p>
                    </div>
                    <div class="col-6">
                        <p><strong>Residencia:</strong><br>${visitor.residentId.residenceNumber}</p>
                        <p><strong>Fecha:</strong><br>${new Date(visitor.visitDate).toLocaleDateString('es-ES')}</p>
                        <p><strong>Hora:</strong><br>${visitor.visitTime}</p>
                    </div>
                </div>
                <div class="row mt-3">
                    <div class="col-12">
                        <p><strong>Motivo:</strong><br>${visitor.visitReason}</p>
                    </div>
                </div>
                ${visitor.visitedAt ? `
                <div class="alert alert-success mt-3">
                    <i class="fas fa-check"></i> Visitó el: ${new Date(visitor.visitedAt).toLocaleString('es-ES')}
                </div>
                ` : ''}
            `;
